...
GPIO::GPIO(GPIO_TypeDef *_port, uint8_t _pin, bool isOutput):port(_port), pin(_pin)
{
	...
	if(_port == GPIOA) {RCC->AHB1ENR |= GPIOAEN;}
	if(_port == GPIOB) {RCC->AHB1ENR |= GPIOBEN;}
	if(_port == GPIOC) {RCC->AHB1ENR |= GPIOCEN;}

	/*Configure pin*/
	if(isOutput)
	{
		/*Set mode as output 01*/
		_port->MODER |=  (1<<(2*_pin));
		_port->MODER &=~ (1<<(2*_pin + 1));
	}
	else
	{
		/*Set mode as input 00*/
		_port->MODER &=~ (3<<(2*_pin + 1));
	}
}
